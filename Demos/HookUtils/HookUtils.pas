...
//功能：计算需要覆盖的机器指令大小，借助了 BeaEngine 反汇编引擎以免指令被从中间切开
//参数：
////////////////////////////////////////////////////////////////////////////////
function CalcHookProcSize(AFunc: Pointer): Integer;
const
{$IFDEF CPUX64}
  defDisasmArchi: UInt32 = ARCHI_X64; // for x64 ...
{$ELSE}
  defDisasmArchi: UInt32 = ARCHI_X32; // for x32 ...
{$ENDIF}
var
  aDisasm: TDisasm;
  bEnough: Boolean;
...
begin
  ...
  nMinSize := SizeOf(TNewProc);
  nRet := 0;
  bEnough := False;
  pCode := PByte(AFunc);

  while not bEnough do
  begin
    FillChar(aDisasm, SizeOf(aDisasm), 0);
    aDisasm.Archi := defDisasmArchi;
    aDisasm.EIP := UIntPtr(pCode);
    aDisasm.Options := MasmSyntax;
    aDisasm.SecurityBlock := $20;
